...
To run this Runchat, you’ll need to first [authorize Google Sheets](https://www.notion.so/Read-and-Write-to-Google-Sheets-17f2742769ce8055930ddcd04fb417e8?pvs=21) from the Runchat Dashboard. The runchat itself is very simple - we have a `Webhook` node connected to the data input of the `Write Sheet` node from the Google Sheets library. 
